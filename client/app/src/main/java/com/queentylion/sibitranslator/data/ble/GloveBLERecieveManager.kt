...
class GloveBLERecieveManager @Inject constructor(
    ...
    private val DEVICE_NAME = "Glove-Flex-Sensors"
        ...
        override fun onConnectionStateChange(gatt: BluetoothGatt, status: Int, newState: Int) {
            if(status == BluetoothGatt.GATT_SUCCESS){
                if(newState == BluetoothProfile.STATE_CONNECTED){
                    ...
                    coroutineScope.launch {
                        data.emit(Resource.Success(data = GloveResult(FloatArray(5),ConnectionState.Disconnected)))
                    }
                    gatt.close()
                }
            }else{
                ...
                when(uuid){
                    UUID.fromString(TEMP_HUMIDITY_CHARACTERISTICS_UUID) -> {
                        ...
                        val tempHumidityResult = GloveResult(
                            // TO BE CHANGED WITH 5 VALUE FROM UUID
                            FloatArray(5) {
                                5f;5f;5f;5f;5f
                            },
